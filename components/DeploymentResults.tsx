...
export default function DeploymentResults({ endpointUrl, metadata }: DeploymentResultsProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="border-green-700 bg-green-950/20">
        <CardContent className="pt-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-green-400">🎉 Fractalic AI Server deployed successfully!</h3>
              <p className="text-sm text-gray-300">Your deployment is ready and accessible</p>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="space-y-2 text-sm">
            ...
            {metadata.api && metadata.api.endpoint && (
              <div className="flex justify-between items-center">
                ...
                <span className="text-white font-mono">{metadata.api.endpoint}</span>
              </div>
            )}
          </div>
        </CardContent>
      ...
      {/* Sample Usage */}
      {metadata.api && metadata.api.sample_curl && (
        <Card className="border-gray-700 bg-[#252525]">
          <CardHeader className="pb-3">
          ...
          <CardContent className="space-y-3">
            <div className="space-y-2 text-sm">
              {metadata.container.logs_command && (
                <div className="flex justify-between items-start gap-2">
                  <span className="text-gray-400 flex-shrink-0">• View logs:</span>
                  <div className="flex items-center gap-2">
                    <code className="text-white bg-gray-800 px-2 py-1 rounded text-xs">
                      {metadata.container.logs_command}
                    </code>
              ...
              {metadata.container.remove_command && (
                <div className="flex justify-between items-start gap-2">
                  <span className="text-gray-400 flex-shrink-0">• Remove:</span>
                  <div className="flex items-center gap-2">
                    <code className="text-white bg-gray-800 px-2 py-1 rounded text-xs">
                      {metadata.container.remove_command}
                    </code>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-2 text-sm">
              {metadata.services.ai_server && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-400">• AI Server:</span>
                  <div className="flex items-center gap-2">
              ...
              {metadata.services.mcp_manager && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-400">• MCP Manager:</span>
                  <div className="flex items-center gap-2">
                    {isServiceHealthy(metadata.services.mcp_manager) ? (
                      <>
                        <span className="text-green-400">✅ Healthy (internal)</span>
                        <Badge variant="outline" className="text-green-400 border-green-400 text-xs">
                          Online
                        </Badge>
                      </>
